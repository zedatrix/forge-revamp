...
from .models.ArtifactModel import ArtifactModel
...
def convert_to_task(task_obj: TaskModel, debug_enabled: bool = False) -> Task:
    ...
    status = Status.completed if task_obj.status == "completed" else Status.created
    return Task(
...
class AgentDB:
    ...
    async def update_task(
            ...
            if self.debug_enabled:
                LOG.debug(f"Updating task with task_id: {task_id}")
            try:
                with self.Session() as session:
                    if (
                        ...
                        if output is not None:
                            task.output = output
                        if task is not None:
                            task.additional_output = additional_output
                        session.commit()
                        return await self.get_task(task_id)
                    else:
                        LOG.error(
                            f"Task not found for update with task_id: {task_id}"
                        )
                        raise NotFoundError("Task not found")
            except SQLAlchemyError as e:
                LOG.error(f"SQLAlchemy error while getting task: {e}")
